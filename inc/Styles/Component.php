...
class Component implements Component_Interface, Templating_Component_Interface {
	...
	public function action_enqueue_styles() {
		...
		if ( ! empty( $google_fonts_url ) ) {
			wp_enqueue_style( 'uc-execution-fonts', $google_fonts_url, [], null ); // phpcs:ignore WordPress.WP.EnqueuedResourceParameters.MissingVersion
		}
		...
		foreach ( $css_files as $handle => $data ) {
			...
			$version = uc_execution()->get_asset_version( $css_dir . $data['file'] );
	...
	public function filter_resource_hints( array $urls, string $relation_type ) : array {
		if ( 'preconnect' === $relation_type && wp_style_is( 'uc-execution-fonts', 'queue' ) ) {
			$urls[] = [
		...
		$handles   = array_filter(
			...
			function( $handle ) use ( $css_files ) {
				...
				if ( ! $is_valid ) {
					...
					_doing_it_wrong( __CLASS__ . '::print_styles()', esc_html( sprintf( __( 'Invalid theme stylesheet handle: %s', 'uc-execution' ), $handle ) ), 'UC Execution 2.0.0' );
				}
				return $is_valid;
			}
		);
	 ...
	 * {@see 'uc_execution_preloading_styles_enabled'} filter can be used to tweak the return value.
	 *
	...
	protected function preloading_styles_enabled() {
		$preloading_styles_enabled = ! uc_execution()->is_amp();
		 ...
		 */
		return apply_filters( 'uc_execution_preloading_styles_enabled', $preloading_styles_enabled );
	}
	...
	protected function get_css_files() : array {
		...
		$css_files = [
			...
			'uc-execution-comments'   => [
				'file'             => 'comments.min.css',
				'preload_callback' => function() {
					return ! post_password_required() && is_singular() && ( comments_open() || get_comments_number() );
				},
			],
			'uc-execution-content'    => [
				'file'             => 'content.min.css',
				'preload_callback' => '__return_true',
			],
			'uc-execution-sidebar'    => [
				'file'             => 'sidebar.min.css',
				'preload_callback' => function() {
					return uc_execution()->is_primary_sidebar_active();
				},
			],
			'uc-execution-widgets'    => [
				'file'             => 'widgets.min.css',
				'preload_callback' => function() {
					return uc_execution()->is_primary_sidebar_active();
				},
			],
			'uc-execution-footer-widgets'    => [
				'file'             => 'footer-widgets.min.css',
				'preload_callback' => function() {
					return uc_execution()->is_footer_widgets_active();
				},
			],
			'uc-execution-homepage' => [
				'file' => 'homepage.min.css',
				'preload_callback' => function() {
					global $template;
					return 'front-page.php' === basename( $template );
				},
			],
			'uc-execution-related'    => array(
		 ...
		 */
		$css_files = apply_filters( 'uc_execution_css_files', $css_files );
	...
	protected function get_google_fonts() : array {
		 ...
		 */
		$this->google_fonts = (array) apply_filters( 'uc_execution_google_fonts', $google_fonts );
