...
class Component implements Component_Interface {
	...
	public function action_enqueue_navigation_script() {
		...
		wp_enqueue_script(
			'uc-execution-navigation',
			get_theme_file_uri( '/assets/js/navigation.min.js' ),
			[],
			uc_execution()->get_asset_version( get_theme_file_path( '/assets/js/navigation.min.js' ) ),
			false
		);
		wp_script_add_data( 'uc-execution-navigation', 'async', true );
		wp_script_add_data( 'uc-execution-navigation', 'precache', true );
		wp_localize_script(
			'uc-execution-navigation',
			'ucExecutionScreenReaderText',
			[
				'expand'   => __( 'Expand child menu', 'uc-execution' ),
				'collapse' => __( 'Collapse child menu', 'uc-execution' ),
			]
		);
	}
